...
SED_data = [l.strip('\n').split('\t') for l in open('../SED_data/SED_cognates.tsv','r')]

ancient = ['Ugaritic','Akkadian'] #ancient languages that are also in phylogeny

for i,l in enumerate(SED_data):
    if len(l) == 7:
        ll = l
        ll[5:7] = [' '.join(ll[5:7])]
        SED_data[i] = ll
...
for i,l in enumerate(SED_data):
    SED_data[i][2] = SED_data[i][2].split(' - ')[0]

#SED_data = [l for l in SED_data if l[4] != '']

SED_split = defaultdict(list)
for l in SED_data:
    w = l[4]
    w = re.sub(r'.+-: ','',w)
    w = re.sub('\(.*\)','',w)
    ...
    w_ = re.split(r', |~|:|/',w)
    for s in w_:
        SED_split[tuple(l)].append(s)

SED_data = []
for key in SED_split.keys():
    l = list(key)
    for v in SED_split[key]:
        SED_data.append(l+[v])

SED_data = [l for l in SED_data if l[6] != '']

SED_data = SED_data[1:]

segs_to_strip = ['(', ')', '*', ',', '-', '/', '<', '>', '?','₁', '₂', '₃', '₄', '₅', '₆', '₇', '₈', '₉','-',' ']

reflex = [list(l[6]) for l in SED_data]

etymon = [list(re.sub('['+'|'.join(segs_to_strip)+']','',re.sub(r'\([^\)]*\)','',l[2]))) for l in SED_data]
...
for i in range(len(etymon)):
    if SED_data[i][3] not in ancient:
        reflex_split = re.split(r'[ |-]',''.join(reflex[i]))
        reflex_split = [s for s in reflex_split if s != '']
    else:
        reflex_split = re.split(r'\s',''.join(reflex[i]))
        reflex_split = [s.replace('-','') for s in reflex_split if s != '']
    scores = []
    for seg in reflex_split:
        scores.append(NWscore(S,etymon[i],list(seg)))
    j = np.argmax(scores)
    SED_data[i].append(reflex_split[j])

f = open('SED_merged_reflex_aligned.tsv','w')
for l in SED_data:
    print('\t'.join(l),file=f)
