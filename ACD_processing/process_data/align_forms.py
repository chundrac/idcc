"""This script aligns etyma in the Austronesian Comparative Dictionary with the portions of their reflexes most likely to descend from them. In many cases, reflexes consist of multi-word and other expressions of which only a portion descends from the etymon of interest. We use the alignment algorithm of Jäger (2013) to isolate the truly homologous form.
...
acd_split = defaultdict(list)
for l in acd_data[1:]:
    w = l[3]
    w = re.sub(' \(.*\)','',w)
    w_ = re.split(r', | ~ ',w)
    for s in w_:
        acd_split[tuple(l)].append(s)

acd_data = [acd_data[0]+['wordform_normalized']]
for key in acd_split.keys():
    l = list(key)
    for v in acd_split[key]:
        acd_data.append(l+[v])

segs_to_strip = ['(', ')', '*', ',', '-', '/', '<', '>', '?','₁', '₂', '₃', '₄', '₅', '₆', '₇', '₈', '₉','-',' ']

reflex = [list(l[10]) for l in acd_data[1:]]
