...
data.full <- read.csv('character_data_string.tsv',row.names=1,sep='\t',check.names = F)
...
plottree <- function(d) {
  ...
  rnames <- paste(tree$tip.label,sub.df.full)
...
while (d <= D) {
  ...
  p <- plottree(d)
  p
  ggsave(filename = paste('../../data_plots/austronesian/',d,'.pdf',sep=''), plot = p, device=cairo_pdf)
  #dev.off()
  d <- d + 1
}
