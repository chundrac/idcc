...
rate.df <- rate.df[c(which(rate.df$fam.ID=='Austronesian' & rate.df$rate <= log(2000)),
                     which(rate.df$fam.ID=='Semitic' & rate.df$rate <= log(3000)),
                     which(rate.df$fam.ID=='Uralic' & rate.df$rate <= log(3500))),]

p <- ggplot(rate.df, aes(x = exp(rate))) + 
  geom_histogram(aes(fill=rate.type),alpha=.6) + 
  scale_fill_manual(guide="none",values=c("#E69F00", "#56B4E9", "#D55E00")) + 
  geom_line(data=intervals.df,aes(x=x,y=y),color='black',alpha=.75) + 
  geom_vline(xintercept = 1,linetype="dashed") + 
  #geom_vline(data=medians.df,aes(xintercept = value),linetype="dotted") + 
  geom_vline(data=medians.df,aes(xintercept = x),color='black',alpha=.75) + 
  facet_grid2(vars(rate.type),vars(fam.ID), scales="free", independent=TRUE) + 
  ylab('Posterior density') + xlab('') + theme_classic()
...
aggregate(rate ~ fam.ID + rate.type, rate.df, FUN=function(x){sort(x)[length(x)*.999]})
...
p <- ggplot(rate.df, aes(x = exp(rate))) + 
  geom_histogram(aes(fill=rate.type),alpha=.6) + 
  scale_fill_manual(values = c("#56B4E9", "#CC79A7", "#E69F00"), guide=F) + 
  geom_vline(xintercept = 1,linetype="dashed") + 
  #geom_vline(data=medians.df,aes(xintercept = value),linetype="dotted") + 
...
p <- ggplot(rate.df, aes(x = exp(rate))) + 
  ...
  geom_vline(xintercept = 1,linetype="dashed") + 
  #geom_vline(data=medians.df,aes(xintercept = value),linetype="dotted") + 
  geom_vline(data=medians.df,aes(xintercept = x),color='darkgray') + 
  geom_line(data=intervals.df,aes(x=x,y=y), color='lightblue') + 
  facet_grid2(vars(rate.type),vars(fam.ID), scales="free", independent=TRUE) + 
  ylab('Posterior density') + xlab('') + theme_classic() + theme(axis.text.y = element_blank(),axis.ticks.y = element_blank())
